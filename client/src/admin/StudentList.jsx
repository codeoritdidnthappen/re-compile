...
const Cohort = ({ num }) => {
  return (
    <>
      {num == 5 && (
        <svg fill="var(--color-accent)" width="800px" height="800px" viewBox="-1 0 19 19" xmlns="http://www.w3.org/2000/svg" class="cf-icon-svg">
  ...
  return (
    <div className="mt-10 max-w-2xl w-full mx-auto px-4 sm:px-6 lg:px-8">
      <ul className="list bg-base-100 rounded-box shadow-md">
        {data.sort((a, b) => new Date(a.releaseDate).getTime() - new Date(b.releaseDate).getTime()).filter((student) => new Date(student.releaseDate).getTime() > Date.now()).map((student) => (
          <li className="list-row">
            {/* <div><img className="size-10 rounded-box" src={new URL(`../assets/woman.svg`, import.meta.url).href}/></div> */}
            <div><WomanSVG /></div>
            <div>
              <div className="text-lg text-secondary">{student.firstName} {student.lastName} #{student.docId}</div>
              <div className="text-xs uppercase font-semibold">Release Date: {student.releaseDate}</div>
            </div>
            <button className="btn btn-square btn-ghost">
              <Cohort num={student.cohort} />
            </button>
            <button className="btn btn-circle btn-ghost">
              <svg className="size-[1.2em]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <g strokeLinejoin="round" strokeLinecap="round" strokeWidth="2" fill={`#${student.docId}`} stroke="none">
